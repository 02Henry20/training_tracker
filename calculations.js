...
function enteredLoadKg(exercise, set) {
...
export function totalExternalLoad(exercise, set) {
  return Math.max(0, enteredLoadKg(exercise, set));
}

function isAssistedBodyweightExercise(exercise) {
  return exercise?.loadMode === "assisted" || ["assisted-pull-up", "assisted-dip-machine"].includes(exercise?.id);
}
...
function bodyweightVolumeFactor(exercise) {
  ...
  if (["dips", "assisted-dip-machine"].includes(exercise?.id)) return 0.95;
  if (exercise?.id === "handstand-push-up") return 0.9;
  if (["decline-push-up", "pike-push-up", "forward-lean-push-up"].includes(exercise?.id)) return 0.75;
  if (["push-up", "diamond-push-up", "archer-push-up"].includes(exercise?.id)) return 0.65;
...
function assistedBodyweightLoadForSet(exercise, set, bodyWeightKg) {
  const bodyLoad = bodyWeightKg * bodyweightVolumeFactor(exercise);
  const assistKg = Math.abs(enteredLoadKg(exercise, set));
  return Math.max(0, bodyLoad - assistKg);
}

function bodyweightRepScore(exercise, set, bodyWeightKg) {
  const reps = Math.max(0, Number(set?.reps) || 0);
  if (isAssistedBodyweightExercise(exercise)) {
    return reps * (assistedBodyweightLoadForSet(exercise, set, bodyWeightKg) / bodyWeightKg);
  }
  const addedKg = Math.max(0, totalExternalLoad(exercise, set));
  return reps + addedKg / bodyWeightKg * 25;
}
...
function volumeLoadForSet(exercise, set, bodyWeightKg) {
  ...
  if (exercise?.inputType === "bodyweightSets") {
    if (isAssistedBodyweightExercise(exercise)) return assistedBodyweightLoadForSet(exercise, set, bodyWeightKg);
...
export function activityMet(exercise, activity = {}, settings = {}) {
  ...
  switch (exercise?.activityType) {
    ...
    case "trailRunning": {
      const terrainBonus = Number.isFinite(speed) ? 0.9 : 1.2;
      return clamp(metForRunning(speed) + terrainBonus + Math.min(5.5, gradeProxy * 24), 6.2, 19.5);
    }
...
function bestStrengthSet(exercise, entry, bodyWeightKg = 70) {
  ...
  if (exercise.inputType === "bodyweightSets") {
    const bodyWeight = Math.max(20, Number(bodyWeightKg) || 70);
    return sets.reduce((best, set) => {
      const score = bodyweightRepScore(exercise, set, bodyWeight);
      const bestScore = bodyweightRepScore(exercise, best, bodyWeight);
...
export function evaluateExerciseLevel(exercise, entry, settings) {
  ...
  const best = bestStrengthSet(exercise, entry, bodyWeight);
  ...
  if (standard.type === "repetitions") {
    if (!best) return { ...LEVELS[0], index: 0, value: null, progress: 0 };
    return levelFromThresholds(bodyweightRepScore(exercise, best, bodyWeight), standard.thresholds);
  }
...
export function analyseExerciseEntry(entry, exercise, settings) {
  ...
  if (exercise.inputType === "activity") {
    ...
    bestSet ??= bestStrengthSet(exercise, entry, bodyWeight);
  }
